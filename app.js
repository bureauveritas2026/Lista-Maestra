...
async function loadData() {
    ...
    if (area) {
        collection = db.documentos.where('area').equals(area);
    }
...
docForm.onsubmit = async (e) => {
    ...
    try {
        if (editModeId) {
            ...
            if (!fileBlob) {
                docData.fileBlob = oldDoc.fileBlob;
                docData.fileName = oldDoc.fileName;
            }
            await db.documentos.update(editModeId, docData);
            showToast("Documento actualizado con éxito");
        } else {
...
async function exportToJSON() {
    const data = await db.documentos.toArray();
    // We need to convert Blobs to base64 for JSON export if we want to include files
    ...
    reader.onload = async (e) => {
        try {
            ...
            for (let item of data) {
                if (item.fileBlob && typeof item.fileBlob === 'string') {
            ...
            showToast("Error al importar el archivo", "danger");
        }
    };
